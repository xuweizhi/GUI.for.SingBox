...
func TestExecBackgroundReusesExistingCoreProcess(t *testing.T) {
	...
	tempDir := t.TempDir()
	prevBasePath := Env.BasePath
	prevRuntimeMode := Env.RuntimeMode
	t.Cleanup(func() {
		Env.BasePath = prevBasePath
		Env.RuntimeMode = prevRuntimeMode
	})
...
func TestExecBackgroundDoesNotReuseExistingCoreProcessInDesktopMode(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("shell-based fake core is only used on non-Windows platforms")
	}
	...
	Env.BasePath = tempDir
	Env.RuntimeMode = RuntimeModeDesktop
	...
	if firstPID == secondPID {
		t.Fatalf("expected desktop mode to start a new process, got reused pid %d", firstPID)
	}
